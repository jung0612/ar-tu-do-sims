...
//int16_t key_state[4];
double key_state[4];
double car_state[2]={0,0};
struct sockaddr_in servaddr, cliaddr;
std_msgs::Float32 car;
...
KeyboardController::KeyboardController()
{
    ...
    this->m_drive_mode_subscriber =
        this->m_node_handle.subscribe<std_msgs::Int32>(TOPIC_DRIVE_MODE, 1, &KeyboardController::driveModeCallback, this);

    this->car_select = this->m_node_handle.advertise<std_msgs::Float32>(CAR_SELECT, 1);
...
//    n = recvfrom(sockfd, key_state, sizeof key_state,
//                 MSG_WAITALL, ( struct sockaddr *) &cliaddr,
//                 &len);
    key_state[0]=0;
    ROS_INFO("%i %i %i %i", key_state[0], key_state[1], key_state[2] ,key_state[3]);
    this->m_angle = key_state[0];
    this->m_velocity = key_state[1];

//    this->m_key_pressed_state = {key_state[0], key_state[1], key_state[2], key_state[3], false, false};
    this->updateWindow();
...
{
    car.data=key_state[2];
    drive_msgs::drive_param drive_parameters;
    drive_parameters.velocity = this->m_velocity;
    drive_parameters.angle = this->m_angle;

//    ROS_INFO("%d %d", this->m_velocity, this->m_angle);
//    car_state[0]=this->m_velocity;
//    car_state[1]=this->m_angle;
//    car_state[0]=1;
//    car_state[1]=2;
//    ROS_INFO("%d",this->m_velocity);
//    std::cout<<this->m_velocity;
//    int ss[2];
//    ss[0]=1;
//    ss[1]=2;
//    ROS_INFO("%i", cliaddr);
//    sendto(sockfd, car_state, sizeof car_state, 0, ( struct sockaddr *) &cliaddr,  len);
//    sendto(sockfd, ss, sizeof ss, 0, ( struct sockaddr *) &cliaddr,  len);

    this->m_drive_parameters_publisher.publish(drive_parameters);
    this->car_select.publish(car);
}
...
//    servaddr.sin_addr.s_addr = inet_addr("163.152.127.103");
